...
const SourceScanner = ({ onScanComplete }) => {
  // AuthContext persists the token in sessionStorage (localStorage fallback for older sessions).
  ...
  return (
    ...
    >
      ...
      <div className="bg-slate-50/70 border-b border-slate-200 px-5 pt-4 pb-0">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-black text-slate-900 text-xs tracking-widest font-mono">
          ...
          <button
            ...
            className="flex items-center gap-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-300 text-[10px] font-black tracking-widest px-3 py-1.5 rounded-lg transition-all active:scale-95 font-mono whitespace-nowrap self-start sm:self-auto"
          >
            {reconciling
              ? <span className="w-2.5 h-2.5 border-2 border-amber-500/30 border-t-amber-600 rounded-full animate-spin" />
              : <GitCompare size={12} />
            }
        ...
        <div className="flex gap-1 overflow-x-auto border-t border-slate-200 pt-2 scrollbar-none">
          {SCAN_MODES.map(mode => {
            ...
            return (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-2 text-[10px] font-bold font-mono tracking-wider transition-colors border-b-2 whitespace-nowrap ${
                  active
                    ? 'text-cobalt-700 border-cobalt-600'
      ...
      <AnimatePresence>
        {reconcileResult && (
          <motion.div
            ...
            className="mx-5 mb-4 p-4 bg-amber-50 border border-amber-300 rounded-xl space-y-3"
          >
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-black font-mono text-amber-800 flex items-center gap-2">
                <GitCompare size={14} /> MULTI-SOURCE RECONCILIATION REPORT
              </span>
              <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-white border border-amber-300 text-amber-700">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-center">
              <div className="bg-white p-2 rounded border border-slate-200">
                ...
                <div className="text-sm font-bold text-slate-900 font-mono">{reconcileResult.total_evaluated}</div>
              </div>
              <div className="bg-white p-2 rounded border border-slate-200">
                <div className="text-[9px] text-slate-400 font-mono">Divergences</div>
                <div className="text-sm font-bold text-red-600 font-mono">{reconcileResult.divergence_count}</div>
              </div>
              <div className="bg-white p-2 rounded border border-slate-200">
                <div className="text-[9px] text-slate-400 font-mono">Updated DB</div>
                <div className="text-sm font-bold text-cobalt-700 font-mono">{reconcileResult.updated_records}</div>
              </div>
              <div className="bg-white p-2 rounded border border-slate-200">
                <div className="text-[9px] text-slate-400 font-mono">Maturity</div>
                <div className="text-sm font-bold text-emerald-600 font-mono">{reconcileResult.crypto_agility_score}/5</div>
              </div>
            </div>
            ...
            {reconcileResult.divergences?.length > 0 && (
              <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                {reconcileResult.divergences.map((div, i) => (
                  <div key={i} className="bg-white border border-red-200 rounded p-2 text-[10px] font-mono space-y-1">
                    <div className="flex justify-between items-center">
                      <span className="text-red-600 font-bold">{div.divergence_flag}</span>
                      <span className="text-[8px] bg-red-50 text-red-600 border border-red-200 px-1 rounded">{div.severity}</span>
                    </div>
          ...
          >
            ...
            <div className="bg-slate-50 rounded-xl border border-slate-200 p-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="flex flex-col">
                ...
                <span className="text-xl font-black text-slate-900 font-mono">{result.assets_found}</span>
              </div>
              <div className="flex flex-col">
